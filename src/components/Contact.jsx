    ...
    <section id="contact" className="max-w-6xl mx-auto px-6 py-20">
      ...
      {submitted ? (
        <div className="border border-signal/40 bg-signal/10 text-ink px-6 py-8 max-w-lg rounded-sm">
          ...
          <p className="text-sm text-ink/85">We'll get back to you shortly. In the meantime feel free to email hello@nexora.io directly.</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="max-w-lg space-y-5">
          <div>
            ...
            <input required id="name" name="name" value={form.name} onChange={handleChange}
              className="w-full border border-ink/35 bg-transparent px-3 py-2 rounded-sm focus:outline-none focus:ring-2 focus:ring-signal" />
          </div>
          <div>
          ...
          <p className="text-sm text-ink/80 pt-1">
